...
// ── Session string decoder (from MD bot pairing sites) ───────────────────────
...
async function importSessionString() {
  ...
  const { data } = await sb.from('wa_session').select('key').eq('key', 'creds').single()
  if (data) return // Already have session in DB

  try {
    // Levanter format: "Levanter;;base64string"
    // Other MD bots: "BOTNAME;;base64string" or just "base64string"
    let raw = SESSION_ID
    if (SESSION_ID.includes(';;')) {
      raw = SESSION_ID.split(';;')[1]
    }
    ...
    // Save creds key
    if (decoded.creds) {
      await sb.from('wa_session').upsert({
        key: 'creds',
        value: JSON.stringify(decoded.creds, BufferJSON.replacer)
      })
      // Save remaining keys
      ...
      // Flat format — entire object is creds
      await sb.from('wa_session').upsert({
        key: 'creds',
        value: JSON.stringify(decoded, BufferJSON.replacer)
      })
    }

    console.log('🦩 Session imported from SESSION_ID successfully')
  } catch (e) {
    console.error('Session import failed:', e.message)
    console.error('Raw value (first 100 chars):', process.env.SESSION_ID?.slice(0, 100))
  }
}
...
export async function useSupabaseAuthState() {
  // Import session string on first run if provided
